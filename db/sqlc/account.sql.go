...
const getUserAccountWithUserFields = `-- name: GetUserAccountWithUserFields :one
SELECT accounts.id, accounts.owner, accounts.balance, accounts.currency, accounts.account_type, accounts.created_at, users.email_address, users.id as user_id
FROM accounts
INNER JOIN users ON accounts.owner = users.id
WHERE accounts.id = $1
`

type GetUserAccountWithUserFieldsRow struct {
	ID           int64     `json:"id"`
	Owner        int64     `json:"owner"`
	Balance      int64     `json:"balance"`
	Currency     string    `json:"currency"`
...
func (q *Queries) GetUserAccountWithUserFields(ctx context.Context, id int64) (GetUserAccountWithUserFieldsRow, error) {
	row := q.db.QueryRowContext(ctx, getUserAccountWithUserFields, id)
	var i GetUserAccountWithUserFieldsRow
	err := row.Scan(
		&i.ID,
		&i.Owner,
		&i.Balance,
...
const getUsersAccountsByUserEmail = `-- name: GetUsersAccountsByUserEmail :many
SELECT accounts.id, accounts.owner, accounts.balance, accounts.currency, accounts.account_type, accounts.created_at, users.email_address
FROM accounts
INNER JOIN users ON accounts.owner = users.id
WHERE users.email_address = $1
`

type GetUsersAccountsByUserEmailRow struct {
	ID           int64     `json:"id"`
	Owner        int64     `json:"owner"`
	Balance      int64     `json:"balance"`
	Currency     string    `json:"currency"`
...
func (q *Queries) GetUsersAccountsByUserEmail(ctx context.Context, emailAddress string) ([]GetUsersAccountsByUserEmailRow, error) {
	rows, err := q.db.QueryContext(ctx, getUsersAccountsByUserEmail, emailAddress)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetUsersAccountsByUserEmailRow{}
	for rows.Next() {
		var i GetUsersAccountsByUserEmailRow
		if err := rows.Scan(
			&i.ID,
			&i.Owner,
			&i.Balance,
